- [Section 11 - 파드 메타데이터와 리소스 액세스 하기](#section-11---파드-메타데이터와-리소스-액세스-하기)
  - [파드 메타데이터와 그외의 리소스에 액세스 하기](#파드-메타데이터와-그외의-리소스에-액세스-하기)
    - [Downward API 로 메타데이터 전달](#downward-api-로-메타데이터-전달)
    - [사용 가능한 메타데이터 이해](#사용-가능한-메타데이터-이해)
      - [환경변수를 활용한 메타데이터 노출하기](#환경변수를-활용한-메타데이터-노출하기)
      - [파일로 메타데이터 전달](#파일로-메타데이터-전달)
      - [레이블과 어노테이션 업데이트](#레이블과-어노테이션-업데이트)
      - [볼륨 스펙에서 컨테이너 수준의 메타데이터 참조](#볼륨-스펙에서-컨테이너-수준의-메타데이터-참조)
  - [쿠버네티스 API 서버와 통신하기](#쿠버네티스-api-서버와-통신하기)
    - [kubectl proxy로 쿠버네티스 API 살펴보기](#kubectl-proxy로-쿠버네티스-api-살펴보기)
    - [앰배서더 컨테이너를 이용한 API 서버 통신 간소화](#앰배서더-컨테이너를-이용한-api-서버-통신-간소화)
  - [클라이언트 라이브러리를 사용해 API 서버와의 통신](#클라이언트-라이브러리를-사용해-api-서버와의-통신)
    - [기본 요약](#기본-요약)
