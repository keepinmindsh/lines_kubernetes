- [Section 10 - 시크릿에 대하여](#section-10---시크릿에-대하여)
  - [Secret](#secret)
    - [Secret](#secret-1)
    - [시크릿 생성하기](#시크릿-생성하기)
    - [개별 시크릿 제한](#개별-시크릿-제한)
    - [시크릿 편집](#시크릿-편집)
    - [시크릿 사용하기](#시크릿-사용하기)
      - [기본 토큰 시크릿 소개](#기본-토큰-시크릿-소개)
      - [시크릿 생성](#시크릿-생성)
      - [시크릿 보륨의 도트 파일](#시크릿-보륨의-도트-파일)
      - [서비스 어카운트 토큰 시크릿](#서비스-어카운트-토큰-시크릿)
      - [도커 컨피그 시크릿](#도커-컨피그-시크릿)
      - [기본 인증(Basic authentication) 시크릿](#기본-인증basic-authentication-시크릿)
      - [SSH 인증 시크릿](#ssh-인증-시크릿)
      - [TLS 시크릿](#tls-시크릿)
      - [시크릿을 불변으로 지정하기](#시크릿을-불변으로-지정하기)
      - [컨피그맵과 시크릿 비교](#컨피그맵과-시크릿-비교)
        - [SSH authentication secrets](#ssh-authentication-secrets)
    - [파드에서 시크릿 사용](#파드에서-시크릿-사용)
      - [Secret 사용 여부 점검](#secret-사용-여부-점검)
      - [시크릿 볼륨을 메모리에 저장하는 이유](#시크릿-볼륨을-메모리에-저장하는-이유)
      - [환경 변수로 시크릿 항목 노출](#환경-변수로-시크릿-항목-노출)
    - [이미지를 가져올 때 사용하는 시크릿 이해](#이미지를-가져올-때-사용하는-시크릿-이해)
      - [도커 허브에서 프라이빗 이미지 사용](#도커-허브에서-프라이빗-이미지-사용)
      - [파드 정의에서 도커레지스트리 시크릿 사용](#파드-정의에서-도커레지스트리-시크릿-사용)
      - [모든 파드에서 이미지를 가져올 때 사용할 시크릿을 모두 지정할 필요는 없다.](#모든-파드에서-이미지를-가져올-때-사용할-시크릿을-모두-지정할-필요는-없다)
